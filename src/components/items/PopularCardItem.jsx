...
const PopularCardItem = ({ productItem }) => {
    ...
    const {title, thumb, author, authorThumb, price} = productItem; 
        ...
        <div className="col-xl-3 col-lg-4 col-sm-6 col-xs-6">
            <div className="popular-item-card">
                <div className="popular-item-card__thumb">
                    <Link to={PopularItemURL}  state={{ thumb, title, author, authorThumb, randomSales, randomRatingText, price }} className="link w-100"> 
                    ...
                    <div className="product-card__bottom flx-between gap-2">
                        ...
                        <span className="product-card__author">
                            by <Link to="/profile" state={{ author, authorThumb }} className="link text-decoration-underline"> 
                                {productItem.author}
                            </Link>
                        </span>
                    </div>
                ...
                <div className="popular-item-card__content d-flex align-items-center justify-content-between gap-2 text-start">
                    <h6 className="popular-item-card__title mb-0">
                        <Link to={PopularItemURL}  state={{ thumb, title, author, authorThumb, randomSales, randomRatingText, price }} className="link"> {productItem.title}</Link>
                    </h6>
                    <Link to={PopularItemURL}  state={{ thumb, title, author, authorThumb, randomSales, randomRatingText, price }} className="btn-link line-height-1 flex-shrink-0">
